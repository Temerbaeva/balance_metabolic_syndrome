...
def sidebar_input_features():
    ...
    bmi = st.sidebar.slider("Индекс массы тела", min_value=1, max_value=80, value=0,
                            step=1)

    waitstcirc = st.sidebar.slider(
        "Окружность талии",
    ...
    hdl = st.sidebar.slider("Уровень холестерина липопротеинов высокой плотности",
                                     min_value=0, max_value=160, value=0, step=1)

    triglycerides = st.sidebar.slider("Уровень триглицерида",
                            min_value=0, max_value=1600, value=0, step=1)
